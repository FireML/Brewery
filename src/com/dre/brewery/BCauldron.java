...
public class BCauldron {
	...
	public static boolean ingredientAdd(Block block, ItemStack ingredient, Player player) {
		...
		if (LegacyUtil.getFillLevel(block) != EMPTY) {
			...
			if (!event.isCancelled()) {
				...
				//Brewery.getInstance().debugLog("Cauldron add: t2 " + ((t2 - t1) / 1000) + " t3: " + ((t3 - t2) / 1000) + " t4: " + ((t4 - t3) / 1000) + " t5: " + ((t5 - t4) / 1000) + "µs");
	...
	public boolean fill(Player player, Block block) {
		if (!player.hasPermission("brewery.cauldron.fill")) {
			Brewery.getInstance().msg(player, Brewery.getInstance().languageReader.get("Perms_NoCauldronFill"));
			return true;
		}
	...
	public static void printTime(Player player, Block block) {
		if (!player.hasPermission("brewery.cauldron.time")) {
			Brewery.getInstance().msg(player, Brewery.getInstance().languageReader.get("Error_NoPermissions"));
			return;
		}
		BCauldron bcauldron = get(block);
		if (bcauldron != null) {
			if (bcauldron.state > 1) {
				Brewery.getInstance().msg(player, Brewery.getInstance().languageReader.get("Player_CauldronInfo1", "" + bcauldron.state));
			} else {
				Brewery.getInstance().msg(player, Brewery.getInstance().languageReader.get("Player_CauldronInfo2"));
			}
		}
	}
	...
	public void cookEffect() {
		if (BUtil.isChunkLoaded(block) && LegacyUtil.isCauldronHeatsource(block.getRelative(BlockFace.DOWN))) {
			...
			if (Brewery.getInstance().use1_13 && particleRandom.nextFloat() > 0.4) {
	...
	public Color getParticleColor() {
		...
		//Brewery.getInstance().log("RGB: " + particleColor.getRed() + "|" + particleColor.getGreen() + "|" + particleColor.getBlue());
		return particleColor;
	}
		...
		if (LegacyUtil.isCauldronHeatsource(down)) {
			...
			if (Brewery.getInstance().use1_9) {
				if (event.getHand() == EquipmentSlot.HAND) {
					...
					plInteracted.add(id);
					Brewery.getInstance().getServer().getScheduler().runTask(Brewery.getInstance(), () -> plInteracted.remove(id));
			...
			if (!player.hasPermission("brewery.cauldron.insert")) {
				Brewery.getInstance().msg(player, Brewery.getInstance().languageReader.get("Perms_NoCauldronInsert"));
				return;
			}
	...
	public static void giveItem(final Player player, final ItemStack item) {
		Brewery.getInstance().getServer().getScheduler().runTaskLater(Brewery.getInstance(), () -> player.getInventory().addItem(item), 1L);
	}
