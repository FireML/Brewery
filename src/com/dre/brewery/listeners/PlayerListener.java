	...
	public void onPlayerInteract(PlayerInteractEvent event) {
		...
		if (type == Material.HOPPER) {
			if (BConfig.brewHopperDump && event.getPlayer().isSneaking()) {
				if (!Brewery.getInstance().use1_9 || event.getHand() == EquipmentSlot.HAND) {
					ItemStack item = event.getItem();
					if (Brew.isBrew(item)) {
						event.setCancelled(true);
						BUtil.setItemInHand(event, Material.GLASS_BOTTLE, false);
						if (Brewery.getInstance().use1_11) {
		...
		if (Brewery.getInstance().use1_14 && BSealer.isBSealer(clickedBlock)) {
			...
			if (BConfig.enableSealingTable) {
				...
				Brewery.getInstance().msg(player, Brewery.getInstance().languageReader.get("Error_SealingTableDisabled"));
			}
			return;
		}
		...
		if (Brewery.getInstance().use1_14 && type == Material.BARREL) {
			...
			if (Brewery.getInstance().use1_14) {
				...
				if ((hand == Material.POTION || hand.isEdible()) && !LegacyUtil.isSign(type)) {
					...
					if (useSlot != -1) {
						...
						Brewery.getInstance().getServer().getScheduler().scheduleSyncDelayedTask(Brewery.getInstance(), () -> player.getInventory().setHeldItemSlot(held), 2);
					}
				}
			...
			if (bplayer != null) {
				...
				switch (bplayer.canJoin()) {
					case 0:
						...
						return;
					case 2:
						event.disallow(PlayerLoginEvent.Result.KICK_OTHER, Brewery.getInstance().languageReader.get("Player_LoginDeny"));
						return;
					case 3:
						event.disallow(PlayerLoginEvent.Result.KICK_OTHER, Brewery.getInstance().languageReader.get("Player_LoginDenyLong"));
				}
			}
		}
	}
