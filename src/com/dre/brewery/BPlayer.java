...
public class BPlayer {
	...
	public static BPlayer getByName(String playerName) {
		if (Brewery.getInstance().useUUID) {
			for (Map.Entry<String, BPlayer> entry : players.entrySet()) {
				OfflinePlayer p = Brewery.getInstance().getServer().getOfflinePlayer(UUID.fromString(entry.getKey()));
				if (p != null) {
	...
	public static boolean drink(Brew brew, ItemMeta meta, Player player) {
		...
		if (meta != null) {
			Brewery.getInstance().getServer().getPluginManager().callEvent(drinkEvent);
		...
		Brewery.getInstance().stats.forDrink(brew);
	...
	public void showDrunkeness(Player player) {
		try {
			...
			if (sendDrunkenessMessage(player)) {
				Brewery.getInstance().getServer().getScheduler().scheduleSyncDelayedTask(Brewery.getInstance(), () -> sendDrunkenessMessage(player), 40);
	...
	public boolean sendDrunkenessMessage(Player player) {
		...
		b.append(Brewery.getInstance().languageReader.get(hangover ? "Player_Hangover" : "Player_Drunkeness"));
		...
		if (hangover && Brewery.getInstance().use1_11) {
			Brewery.getInstance().getServer().getScheduler().scheduleSyncDelayedTask(Brewery.getInstance(), () -> player.sendTitle("", text, 30, 100, 90), 160);
			return false;
		}
		try {
	...
	public void drinkCap(Player player) {
		...
		if (BConfig.overdrinkKick && !player.hasPermission("brewery.bypass.overdrink")) {
			Brewery.getInstance().getServer().getScheduler().scheduleSyncDelayedTask(Brewery.getInstance(), () -> passOut(player), 1);
		} else {
			addPuke(player, 60 + (int) (Math.random() * 60.0));
			Brewery.getInstance().msg(player, Brewery.getInstance().languageReader.get("Player_CantDrink"));
		}
	}
					...
					if (event.getFrom().getX() != event.getTo().getX() || event.getFrom().getZ() != event.getTo().getZ()) {
						...
						if (entity.isOnGround()) {
							...
							if (time == 0) {
								...
								// like now
								if (Brewery.getInstance().use1_9) { // Pushing is way stronger in 1.9
								...
								Brewery.getInstance().getServer().getPluginManager().callEvent(pushEvent);
	...
	public void passOut(Player player) {
		player.kickPlayer(Brewery.getInstance().languageReader.get("Player_DrunkPassOut"));
		offlineDrunk = drunkeness;
		syncToSQL(false);
	}
	...
	public void join(final Player player) {
		...
		Brewery.getInstance().getServer().getScheduler().runTaskLater(Brewery.getInstance(), () -> login(player), 1L);
	}
		...
		if (drunkeness < 10) {
			...
			if (BConfig.enableWake && !player.hasPermission("brewery.bypass.teleport")) {
				...
				if (randomLoc != null) {
					...
					Brewery.getInstance().msg(player, Brewery.getInstance().languageReader.get("Player_Wake"));
				}
			}
			offlineDrunk = 0;
			syncToSQL(false);
	...
	public void goHome(final Player player) {
		...
		if (homeType != null) {
			...
			if (homeType.equalsIgnoreCase("bed")) {
				...
				Brewery.getInstance().errorLog("Config.yml 'homeType: " + homeType + "' unknown!");
			}
			if (home != null) {
	...
	public static void addPuke(Player player, int count) {
		...
		if (pTasks.isEmpty()) {
			taskId = Brewery.getInstance().getServer().getScheduler().scheduleSyncRepeatingTask(Brewery.getInstance(), BPlayer::pukeTask, 1L, 1L);
		}
	...
	public static void pukeTask() {
		...
		if (pTasks.isEmpty()) {
			Brewery.getInstance().getServer().getScheduler().cancelTask(taskId);
		}
	}
	...
	public static void puke(Player player) {
		...
		item.setMetadata("brewery_puke", new FixedMetadataValue(Brewery.getInstance(), true));
		if (Brewery.getInstance().use1_14) item.setPersistent(false); // No need to save Puke items
	...
	public int getQuality() {
		if (drunkeness == 0) {
			Brewery.getInstance().errorLog("drunkeness should not be 0!");
			return quality;
		}
		if (drunkeness < 0) {
			return quality;
