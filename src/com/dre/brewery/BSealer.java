...
public class BSealer implements InventoryHolder {
	public static final NamespacedKey TAG_KEY = new NamespacedKey(Brewery.getInstance(), "SealingTable");
	...
	public BSealer(Player player) {
		...
		if (inventoryHolderWorking) {
			Inventory inv = Brewery.getInstance().getServer().createInventory(this, InventoryType.DISPENSER, Brewery.getInstance().languageReader.get("Etc_SealingTable"));
		...
		inventory = Brewery.getInstance().getServer().createInventory(this, 9, Brewery.getInstance().languageReader.get("Etc_SealingTable"));
	}
	...
	public void clickInv() {
		contents = null;
		if (task == null) {
			task = Brewery.getInstance().getServer().getScheduler().runTaskTimer(Brewery.getInstance(), this::itemChecking, 1, 1);
		}
	}
		...
		for (int i = 0; i < slotTime.length; i++) {
			if (slotTime[i] > 20) {
				...
				if (brew != null && !brew.isStripped()) {
					...
					if (playerValid && Brewery.getInstance().use1_9) {
	...
	public static boolean isBSealer(Block block) {
		if (Brewery.getInstance().use1_14 && block.getType() == Material.SMOKER) {
			Container smoker = (Container) block.getState();
			if (smoker.getCustomName() != null) {
				if (smoker.getCustomName().equals("§e" + Brewery.getInstance().languageReader.get("Etc_SealingTable"))) {
					return true;
				} else {
	...
	public static void blockPlace(ItemStack item, Block block) {
		if (item.getType() == Material.SMOKER && item.hasItemMeta()) {
			...
			if ((itemMeta.hasDisplayName() && itemMeta.getDisplayName().equals("§e" + Brewery.getInstance().languageReader.get("Etc_SealingTable"))) ||
	...
	public static void registerRecipe() {
		...
		ItemMeta meta = Brewery.getInstance().getServer().getItemFactory().getItemMeta(Material.SMOKER);
		if (meta == null) return;
		meta.setDisplayName("§e" + Brewery.getInstance().languageReader.get("Etc_SealingTable"));
		meta.getPersistentDataContainer().set(TAG_KEY, PersistentDataType.BYTE, (byte)1);
		sealingTableItem.setItemMeta(meta);

		ShapedRecipe recipe = new ShapedRecipe(new NamespacedKey(Brewery.getInstance(), "SealingTable"), sealingTableItem);
		recipe.shape("bb ",
					"ww ",
					"ww ");
		...
		Brewery.getInstance().getServer().addRecipe(recipe);
	}

	public static void unregisterRecipe() {
		recipeRegistered = false;
		//Brewery.getInstance().getServer().removeRecipe(new NamespacedKey(Brewery.getInstance(), "SealingTable"));    1.15 Method
		Iterator<Recipe> recipeIterator = Brewery.getInstance().getServer().recipeIterator();
